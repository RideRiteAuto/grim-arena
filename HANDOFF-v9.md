# Gaylinor (Grim Arena) — Project Handoff
...
## Files
- `Grim Arena.dc.html` — THE source file. Everything (template UI + game logic) lives here (~3,900 lines).
...
## Architecture (all inside Grim Arena.dc.html)
...
- The logic is one `Component extends DCLogic` class. Key subsystems (grep for these method names):
  ...
  - Quests (`quests` array + tracker HUD): Daily PMs (10 goblins), track & kill Mr. Saylors, mine 10 ore → smelt 10 bars (furnace) → forge GRIM CLEAVER (anvil).
  - Mounts: killing Mr. Saylors grants donkey riding (hotkey labeled on HUD; lost on death). Seat offset 0.42.
  - NPCs: Paul Bellinger (quest giver, spawn), Alexis Ayala ("buying gf"), roaming knights (Austin Little — easy, Steven Carrasco, Kevin Coelho), non-aggressive goblins, ambient woodcutter/miner workers with periodic chatter, Mr. Saylors (donkey-riding snare-caster, gibberish shouts), Plague Rat boss (bog, poison).
...
## Rebuilding
Edit `Grim Arena.dc.html`, then bundle it into `grim-arena-standalone.html` (self-contained inline build) and distribute that file.
