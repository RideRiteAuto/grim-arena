...
(async () => {
  ...
  await page.goto(BASE + 'furnace.html', { waitUntil: 'load', timeout: 60000 });
  for (let i = 0; i < 40; i++) {
  ...
  const res = await page.evaluate(async (secs) => {
    ...
    h.setVolume(1);
    window.__kit.pour(oc, oc.destination, { at: secs * 0.6, gain: 0.6 });
  ...
  fs.writeFileSync(OUT + '/waveform.svg',
    ...
    '<text x="8" y="16" fill="#8f8f8f" font-family="monospace" font-size="12">furnace, ' +
  ...
  if (perSec < 0.2) fail.push('too few pops: ' + perSec.toFixed(2) + '/s');
