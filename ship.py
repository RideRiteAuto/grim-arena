...
def rebase_onto_origin():
    ...
    for rng in (base + '..HEAD', base + '..origin/master'):
        for f in run('git', 'diff', '--name-only', rng).stdout.split():
            # the bundles are rebuilt from the merged source, and version.json
            # is regenerated from this run's build id. Merging either would be
            # merging an output.
            if f not in ('index.html', 'grim-arena-standalone.html', 'version.json'):
                changed.add(f)
    ...
    # re-stamp AFTER the merge: whichever side's stamp survived, this run's
    # build id is the one that is about to be deployed, and it has to match the
    # version.json players poll or nobody is ever told about the update.
    merged_src = STAMP_RE.sub("var GRIM_BUILD = '%s';" % BUILD[0], merged_src, count=1)
    io.open(SRC, 'w', encoding='utf-8').write(merged_src)
    io.open(VERSION, 'w', encoding='utf-8').write(
        json.dumps({'build': BUILD[0], 'note': MESSAGE[0]}, indent=1) + '\n')
